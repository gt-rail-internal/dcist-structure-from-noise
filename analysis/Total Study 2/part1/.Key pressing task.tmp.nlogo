...
to setup
  ...
  let graph-choice 0  set prevkey -1
  (
  ...
  let uname (user-input "Enter your Mechanical Turk Worker ID:")
  set timelog word timelog uname
  set timelog word timelog "::time, "
  set total-count 1500
...
to initialize-view
 ...
 ; ask patch 2 1 [set plabel "L     "]
  ask patch 2 -1 [set plabel "Click here before pressing keys"]
end
...
;; function to randomly sample 5 clusters of 3 elements each
;; these are the clusters in the lattice graph
;; reports a 5x3 list of the clusters
to-report lattice-clusters
  let rand shuffle n-values 15 [i -> i]
  let clusters (list sublist rand 0 3 sublist rand 3 6 sublist rand 6 9 sublist rand 9 12 sublist rand 12 15)
  report clusters
end

;; reports an adjacency list representation of a lattice graph
;; from the list of clusters that were generated using lattice-clusters
to-report lattice-graph [clusters]
  let graph n-values 15 [[]]
  foreach clusters [cluster ->
    foreach cluster [node ->
      set graph replace-item node graph (remove node cluster)
  ...
  foreach (range 5) [i ->
    foreach (range 3) [j ->
      let node (item j (item i clusters))
      let predeccessor (item j (item ((i - 1) mod 5) clusters))
      let successor (item j (item ((i + 1) mod 5) clusters))
      let adjacent (sentence (item node graph) predeccessor successor)
      set graph (replace-item node graph adjacent)
    ]
  ]
  show graph
  report graph
