...
to setup
  ...
  set user-invalid true
  set second-attempt false
  set initialized false
  ifelse length uname != 0[
    set user-invalid false
    initialize-view
  	initialize-error-params
  ][
    user-message "Sorry! Please enter a valid Mechanical turk ID"
  ]
  set timelog word timelog uname
  set timelog word timelog "::time, "
  set total-count 700
  reset-ticks
end

to initialize-error-params
  set error-window 6
  set error-thresh 3
  set error-count 0
  set error-list []
...
to-report togo-count
  report total-count - press-count
end

to-report delta-time
  report currtime - prevtime
end
...
to initialize-view
 ; ask patch -2 1 [set plabel "H    "]
 ; ask patch -1 1 [set plabel "J     "]
 ; ask patch 0 1 [set plabel "K     "]
 ; ask patch 1 1 [set plabel "L     "]
 ; ask patch 2 1 [set plabel "L     "]
 ask patch 1 -1 [set plabel "Click here before pressing keys"]
end
...
to clear-view-labels
  ...
  [
    ...
    if not second-attempt [
      ...
      set key-status word key-status ", 0"
      set keylog word keylog ", 2"
      show-key
    ]
  ]
end
...
to setup-turtles
  create-turtles 4 [setxy (-2 + who) 0]
  ...
  set keymap [[0][1][2][3][0 1][0 2][0 3][1 2][1 3][2 3]]
  set key 2
  show-key
end
...
to-report modular-clusters
  let rand shuffle n-values 10 [i -> i]
  let clusters (list sublist rand 0 5 sublist rand 5 10)
  show clusters
  report clusters
end
...
to-report modular-graph [clusters]
  ...
  foreach (range 2) [i ->
    ...
    set graph (replace-item node-1 graph (replace-item 3 adj-list-1 (item 4 (item ((i + 1) mod 2) clusters))))
    let node-last (item 4 (item i clusters))
    let adj-list-last (item node-last graph)
    set graph (replace-item node-last graph (replace-item 0 adj-list-last (item 0 (item ((i - 1) mod 2) clusters))))
  ]
  show graph
	report graph
end
...
to update-wrong [current-node mode]
  set keylog (word keylog ", " current-node)
  set key-status (word key-status ", " mode)
  set error-list sentence error-list mode
  if mode = 0
  [
...
to update [current-node]
  ...
  if press-count >= total-count [
    set initialized false
    set completed true
    set token get-token
    set keylog (word keylog "::graph, " adj "::token, " token key-status)
...
to update-time
  let time-now date-and-time
  let hh read-from-string substring time-now 0 2
  let mm read-from-string substring time-now 3 5
  let ss read-from-string substring time-now 6 8
  let ms read-from-string substring time-now 8 12
  set timelog word timelog ", "
  set timelog word timelog time-now
  ...
  set simul currtime - prevtime < 0.090
end
...
to terminated-message
  user-message "Sorry!\n The task has been terminated because you made too many errors. "
end

to only-once-message
  user-message "Sorry! You're allowed to attempt the task only once"
end
...
to press-space
    ...
    (ifelse key = 0 [
      ...
      key = 4 and prevkey = 1 and simul[
        update 4
      ]
      key = 5 and prevkey = 2 and simul[
        update 5
      ]
      key = 6 and prevkey = 3 and simul[
        update 6
      ]
      [
        (ifelse (not (key = 0 or key = 4 or key = 5 or key = 6)) [
...
to press-j
  (ifelse initialized [
    ...
    (ifelse key = 1[
      ...
      key = 4 and prevkey = 0 and simul[
        update 4
      ]
      key = 7 and prevkey = 2 and simul[
        update 7
      ]
      key = 8 and prevkey = 3 and simul[
        update 8
      ]
			[
        (ifelse (not (key = 1 or key = 4 or key = 7 or key = 8)) [
          ask patch 0 -1 [set plabel "Error!"]
          update-wrong 1 0
          ]
          prevkey != -1 [
...
to press-k
  (ifelse initialized [
    ...
    (ifelse key = 2[
      ...
      key = 7 and prevkey = 1 and simul[
        update 7
      ]
      key = 9 and prevkey = 3 and simul[
        update 9
      ]
			[
        (ifelse (not (key = 2 or key = 5 or key = 7 or key = 9)) [
          ask patch 0 -1 [set plabel "Error!"]
          update-wrong 2 0
        ]
        prevkey != -1 [
    ...
    (ifelse key = 3[
      ...
      key = 9 and prevkey = 2 and simul[
        update 9
			...
			[
        (ifelse (not (key = 3 or key = 6 or key = 8 or key = 9)) [
          ask patch 0 -1 [set plabel "Error!"]
          update-wrong 3 0
        ]
        prevkey != -1 [
