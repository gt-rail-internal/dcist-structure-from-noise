# this file performs the same function as the data-extract.py file, but it 
# includes the wrong transitions when computing reaction time. thus reaction times are
# calculated as diff of time at previous response and current response. Insead of the prev 
# correct response and current correct response (as in data-extract.py)
...
rt_writer = csv.writer(open('rt-small-graph-h-04.csv', 'w', newline='\n'))
trial_writer = csv.writer(open('trial-small-graph-h-04.csv', 'w', newline='\n'))
target_writer = csv.writer(open('target-small-graph-h-04.csv', 'w', newline='\n'))
graph_type_writer = csv.writer(open('graph-type-small-graph-h-04.csv', 'w', newline ='\n'))
cluster_writer = csv.writer(open('cluster-small-graph-h-04.csv', 'w', newline='\n'))
graph_writer = csv.writer(open('graph-small-graph-h-04.csv', 'w', newline='\n'))
...
for row in data:
    if row['pretest_key_status'] and len(row['pretest_key_status']) > 0:
        ...
        if len(key_status) >= 700 and ('rail_' in row['user_id'] or row['user_id'] in H_users):
            ...
            correct = [(i, time[i], key_status[i]) for i, x in enumerate(key_status) if x == 1 or x == 0]
            rt = [(correct[i][0], (correct[i][1] - correct[i-1][1])*1000) for i in range(1, len(correct)) if correct[i][2] == 1]
            # a, b = zip(*wrong)
            # plt.ylim(-0.5, 5)
